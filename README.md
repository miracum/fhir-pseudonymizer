...
docker run --rm -i -p 8080:8080 \
  ...
  ghcr.io/miracum/fhir-pseudonymizer:v2.34.0
...
| `MaxRequestBodySize`                  | The maximum allowed size, in bytes, of incoming HTTP request bodies, e.g. to allow processing larger FHIR Bundles. Left unset, Kestrel's own default of `30000000` bytes applies. Note that setting the `ASPNETCORE_Kestrel__Limits__MaxRequestBodySize` environment variable directly has no effect; use this setting instead.                                                                                                                                                                        | `null`                      |
...
```sh
IMAGE=ghcr.io/miracum/fhir-pseudonymizer:v2.34.0
