...
A message's offset is only committed once the broker has acknowledged the message it was turned into - on the output or the dead letter topic - and those of all earlier messages of the same partition, too. This way, a crash or restart never skips a message, though it may process some of them twice. If even publishing a message to the dead letter topic fails, the application stops instead of moving past it, so that it is reprocessed after a restart.
