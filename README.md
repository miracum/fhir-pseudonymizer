...
If `Kafka__ProvenanceTopic` is set, every successfully pseudonymized resource - whether pseudonymized via the Kafka consumer above or via the REST `$de-identify` endpoint - also produces a [Provenance](https://hl7.org/fhir/R4/provenance.html) audit trail: one Provenance resource per pseudonymized resource (or, for a Bundle, one per contained resource), targeting it by its post-pseudonymization `<type>/<id>` (since e.g. cryptoHash-ing `Resource.id` changes it), all wrapped in a single `transaction` Bundle and published to that topic. Each Provenance's `activity` reflects the specific de-identification operations actually applied to that resource (e.g. `REDACTED`, `CRYTOHASH`, `PSEUDED`, ...), read back from the same security label tags the anonymizer already adds to `Meta.security` - diffed against the resource's labels before pseudonymization, so labels the input resource already carried (e.g. from an earlier pseudonymization pass upstream) are never misattributed to this run. It is left unset if nothing new was applied, e.g. when `Anonymization__ShouldAddSecurityTag` is disabled.

Each Provenance entry is a `PUT` to `Provenance/<id>`, where `<id>` is the SHA-256 hash (lowercase hex) of the source resource's first identifier, formatted as `identifier.system|identifier.value`. This makes re-pseudonymizing the same source resource idempotent: the resulting Provenance always gets the same id, so applying the bundle to a FHIR server upserts the existing record instead of accumulating duplicates. If the resource has no identifier, its Provenance falls back to a random id (so it will no longer be idempotent for that resource).

For messages read from Kafka, the provenance bundle is keyed with the original message's key and carries its headers (e.g. tracing context); for REST calls it is produced without a key. A failure to produce the provenance bundle is logged but does not affect the pseudonymized resource being returned/delivered. Producing provenance bundles from the REST API requires `Kafka__Client__BootstrapServers` to be set even if `Kafka__Topics` (Kafka consuming) is left unset.
