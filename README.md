...
| `Anonymization__CryptoHashKeyContext` | If set, `Anonymization__CryptoHashKey` is treated as an HKDF (RFC 5869) master key instead of being used directly, and this value is used as its "info" parameter to derive the actual crypto hash key. Lets a single master key yield many independent, non-reversible keys - e.g. one per project or tenant - without provisioning and storing a separate `CryptoHashKey` for each. The same `(CryptoHashKey, CryptoHashKeyContext)` pair always derives the same key.                                  | `""`                        |
