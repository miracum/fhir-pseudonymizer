...
Messages are parsed and serialized using the System.Text.Json-based FHIR serializer. It validates resources while parsing them, so a message that isn't valid FHIR (e.g. a resource with an id containing characters FHIR doesn't allow, or missing required elements) is published to the dead letter topic.
