...
If `Kafka__ProvenanceTopic` is set, every successfully pseudonymized resource - whether pseudonymized via the Kafka consumer above or via the REST `$de-identify` endpoint - also produces a [Provenance](https://hl7.org/fhir/R4/provenance.html) audit trail: a single Provenance resource targeting every pseudonymized resource (for a Bundle input, one target per contained resource; otherwise just the one resource), each by its post-pseudonymization `<type>/<id>` (since e.g. cryptoHash-ing `Resource.id` changes it), wrapped in a single `transaction` Bundle and published to that topic. The Provenance's `activity` reflects the union of all de-identification operations actually applied across its targets (e.g. `REDACTED`, `CRYTOHASH`, `PSEUDED`, ...), read back from the same security label tags the anonymizer already adds to `Meta.security` - diffed against each resource's labels before pseudonymization, so labels the input already carried (e.g. from an earlier pseudonymization pass upstream) are never misattributed to this run. It is left unset if nothing new was applied, e.g. when `Anonymization__ShouldAddSecurityTag` is disabled.

The Provenance is a `PUT` to `Provenance/<id>`, where `<id>` is the SHA-256 hash (lowercase hex) of its targets' identity, joined together: for each target this is its pre-pseudonymization `<type>/<Resource.id>` if it had one, otherwise its `identifier.system|identifier.value` (using its first identifier). This makes re-pseudonymizing the same input idempotent: the resulting Provenance always gets the same id, so applying the bundle to a FHIR server upserts the existing record instead of accumulating duplicates. Targets with neither an id nor an identifier are skipped when deriving the id; if none of them have either, the Provenance falls back to a random id (so it will no longer be idempotent for that input).
