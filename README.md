...
| `Anonymization__KeyDerivationContext` | If set, `Anonymization__CryptoHashKey` and `Anonymization__EncryptKey` are each treated as their own HKDF (RFC 5869) master key instead of being used directly - the two are derived independently, not from one another - and this value is used as the HKDF "info" parameter to derive each one's actual key. Lets a single master key per algorithm yield many independent, non-reversible keys - e.g. one per project or tenant - without provisioning and storing a separate key for each. The same (master key, `KeyDerivationContext`) pair always derives the same key. | `""`                        |
...
> [!WARNING]
> `Anonymization__CryptoHashKey` and `Anonymization__EncryptKey` should each be a
> randomly generated 32-byte (256-bit) secret. This matches the key size
> HMAC-SHA256 and AES-256 are designed for, whether used directly or as the HKDF
> master key when `Anonymization__KeyDerivationContext` is set. Generate them with:
>
> ```sh
> openssl rand -hex 32
> ```
...
> length. A derived `EncryptKey` (via `KeyDerivationContext`) doesn't have this
> restriction on the master key, since the derived output is always 32 bytes.
