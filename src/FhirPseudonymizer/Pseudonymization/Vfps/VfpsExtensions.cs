...
{
    public static IServiceCollection AddVfpsClient(this IServiceCollection services, VfpsConfig vfpsConfig)
    {
        ...
        services.AddGrpcClient<PseudonymService.PseudonymServiceClient>(o => o.Address = vfpsConfig.Address)
        .ConfigureChannel(o =>
        {
            ...
            o.UnsafeUseInsecureChannelCallCredentials = vfpsConfig.UnsafeUseInsecureChannelCallCredentials;
        })
        .AddCallCredentials((_, metadata) =>
        {
            if (!string.IsNullOrEmpty(vfpsConfig.Auth.Basic.Username))
            {
                var basicAuthString = $"{vfpsConfig.Auth.Basic.Username}:{vfpsConfig.Auth.Basic.Password}";
