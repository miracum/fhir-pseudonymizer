...
namespace Microsoft.Health.Fhir.Anonymizer.Core.Utility
{
    public class DateTimeUtility
    {
        ...
        public static ProcessResult RedactDateTimeAndInstantNode(ElementNode node,
        ...
        public static ProcessResult ShiftDateNode(ElementNode node, string dateShiftKey, string dateShiftKeyPrefix,
            bool enablePartialDatesForRedact = false)
        {
            var processResult = new ProcessResult();
            if (!node.IsDateNode() || string.IsNullOrEmpty(node?.Value?.ToString()))
        ...
        {
            var year = int.Parse(groups[s_yearIndex].Value);
            var month = groups[s_monthIndex].Captures.Any() ? int.Parse(groups[s_monthIndex].Value) : 1;
            var day = groups[s_dayIndex].Captures.Any() ? int.Parse(groups[s_dayIndex].Value) : 1;
